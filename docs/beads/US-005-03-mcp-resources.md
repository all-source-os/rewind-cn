...
## Goal
Expose ralph state as MCP resources for IDE clients to read.

## Tasks
1. In `infrastructure/mcp_server.rs`, implement resource dispatch for `resources/read`:
   - `ralph://backlog` — returns serialized `BacklogProjection` (all task views as JSON)
   - `ralph://epics` — returns serialized `EpicProgressProjection` (all epic progress as JSON)
   - `ralph://config` — reads and returns `.ralph/ralph.toml` as text
...
- `crates/ralph-core/src/domain/model.rs` (modify — add Serialize if needed)
