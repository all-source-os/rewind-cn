...
pub struct RalphMcpServer<B: allframe::cqrs::EventStoreBackend<RalphEvent>> {
    engine: Arc<RalphEngine<B>>,
    config_path: String,
}

impl<B: allframe::cqrs::EventStoreBackend<RalphEvent>> RalphMcpServer<B> {
    pub fn new(engine: Arc<RalphEngine<B>>, config_path: String) -> Self {
        Self {
            engine,
            config_path,
        }
    ...
    pub async fn run(&self) -> Result<(), RalphError> {
        let stdin = io::stdin();
        let mut stdout = io::stdout();

        for line in stdin.lock().lines() {
            let line = line.map_err(|e| RalphError::Storage(format!("stdin read error: {e}")))?;
    ...
    fn write_response(
        ...
        let json = serde_json::to_string(response)
            .map_err(|e| RalphError::Storage(format!("serialize error: {e}")))?;
        writeln!(stdout, "{json}")
            .map_err(|e| RalphError::Storage(format!("stdout write error: {e}")))?;
        stdout
            .flush()
            .map_err(|e| RalphError::Storage(format!("stdout flush error: {e}")))?;
        Ok(())
    }
    ...
    async fn handle_tools_call(&self, id: Option<Value>, params: Value) -> JsonRpcResponse {
        ...
        match tool_name {
            "ralph_status" => self.tool_status(id).await,
            "ralph_plan" => self.tool_plan(id, arguments).await,
            "ralph_run" => self.tool_run(id, arguments).await,
            "ralph_task_list" => self.tool_task_list(id, arguments).await,
            "ralph_task_get" => self.tool_task_get(id, arguments).await,
            _ => JsonRpcResponse::error(id, -32602, format!("Unknown tool: {tool_name}")),
        }
    }

    async fn tool_status(&self, id: Option<Value>) -> JsonRpcResponse {
            ...
            json!({
                "resources": [
                    ...
                    },
                    {
                        "uri": "ralph://config",
                        "name": "Configuration",
                        "description": "ralph.toml configuration",
                        "mimeType": "text/plain"
                    }
                ]
            }),
